...
function workoutForTrainingDay(
  ...
  const finalScale = combinedScale * ruleKmScale;
  ...
  let km = 10;
  let pace = "locker";
  let desc = `Coach-Plan (${dur} Tage bis Rennen).`;

  if (aiRules) {
    const longRunDay = aiRules.longRunDay ?? 0;
    const intervalDay = aiRules.intervalDay ?? 2;
    const tempoDay = aiRules.tempoDay ?? 4;
    if (dow === longRunDay) sessionType = "long";
    else if (dow === intervalDay) sessionType = "interval";
    else if (dow === tempoDay) sessionType = "tempo";
    else sessionType = "easy";
  }

  if (dur <= 10) {
    if (!aiRules) {
      if (dow === 2) sessionType = "interval";
      else if (dow === 0) sessionType = "long";
      else if (dow === 6) sessionType = "easy";
      else sessionType = "easy";
    }
    ...
    desc += " Phase: Rennnah / Taper — Volumen runter.";
  } else if (dur <= 28) {
    if (!aiRules) {
      if (dow === 2) sessionType = "interval";
      else if (dow === 4) sessionType = "tempo";
      else if (dow === 0) sessionType = "long";
      else if (dow === 5) sessionType = "bike";
      else sessionType = "easy";
    }
          ...
          : sessionType === "long"
            ...
            : sessionType === "interval"
              ...
              : sessionType === "tempo"
                ? `Schwellenbereich`
                : `Easy Run`;
    desc += " Phase: Wettkampfspezifikation / Peak-Stich.";
    pace =
      sessionType === "easy"
        ? "locker bis moderat"
        : sessionType === "interval"
    ...
    if (!aiRules) {
      if (dow === 2) sessionType = "interval";
      else if (dow === 4) sessionType = inBuild ? "tempo" : "easy";
      else if (dow === 0) sessionType = "long";
      else if (dow === 5 && dur > 56) sessionType = "strength";
      else if (dow === 6 && dur > 42 && dur !== 63) sessionType = "bike";
      else sessionType = "easy";
    }
            ...
            : sessionType === "long"
              ...
              : sessionType === "interval"
                ...
                : sessionType === "tempo"
                  ? `Schwelle`
                  : `Easy Run`;
    desc += inBuild ? " Phase: Höhere Belastung / Peak." : " Phase: Basis & aerobe Entwicklung.";
  }
