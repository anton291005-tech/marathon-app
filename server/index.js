...
function buildDailyCoachUserPrompt(coachContext) {
  return JSON.stringify({
    ...
    avgFeelingScore: (coachContext.avgRecentFeeling > 0)
...
function pickNumber(source, paths) {
  ...
  for (const path of paths) {
    const value = path.split(".").reduce((obj, key) => (obj && typeof obj === "object" ? obj[key] : undefined), source);
...
function summarizeRecentLoad(context) {
  ...
  const planSessions = Array.isArray(context?.plan) ? context.plan.flatMap((week) => Array.isArray(week?.s) ? week.s : []) : [];
...
function findNextKeySession(context) {
  ...
  const key = sessions.find((session) => ["interval", "tempo", "long", "race"].includes(session?.type)) || sessions[0];
  if (!key) return null;
...
function deriveCoachContext(context) {
  ...
  return {
    ...
    currentPhase: Array.isArray(context?.plan) && context.plan[0]?.phase ? String(context.plan[0].phase) : null,
    recentLoad7d: recent,
    readiness,
    fatigue,
    sleepHours,
...
function detectRiskProfile(userInput, contextSummary) {
  ...
  const hasFatigue = hasFatigueByText
    || (contextSummary?.fatigue !== null && contextSummary?.fatigue >= 0.7)
    || (contextSummary?.readiness !== null && contextSummary?.readiness <= 0.45)
    || (contextSummary?.recentLoad7d?.avgFeeling !== null && contextSummary.recentLoad7d.avgFeeling <= 2.6);
  ...
  console.log({ hasFatigue, wantsIntervals, userMessage });
  return {
    hasIllness,
    hasInjury,
    hasFatigue,
...
function detectImmediateHighRiskOverride(userMessage) {
  ...
  if (hasFatigue && wantsIntervals) {
    return {
      message: "Heute keine Intervalle. Du bist ermuedet und riskierst Ueberlastung. Mach stattdessen einen lockeren Lauf oder nimm einen Ruhetag.",
    };
  }
  if (hasInjury && mentionsRunning) {
    return {
      message: "Kein Laufen. Das Risiko fuer eine Verletzung ist zu hoch. Pause oder alternative Belastung.",
    };
  }
  if (hasIllness && mentionsTraining) {
    return {
      message: "Heute kein Training. Krankheit hat Prioritaet vor Leistung. Nimm Pause und starte erst nach klarer Symptomverbesserung wieder.",
    };
  }
  return null;
}
...
function normalizePreview(rawPreview, actionType, risk, contextSummary) {
  ...
  const items = Array.isArray(rawPreview?.items)
    ...
    : [];
  const safeItems = items.length > 0 ? items : buildDefaultPreviewItems(actionType, risk, contextSummary);
  return {
    title,
    ...
    secondaryLabel: sanitizeSentence(rawPreview?.secondaryLabel) || (actionType === "navigate_to_screen" ? null : "Bearbeiten"),
    openLabel: sanitizeSentence(rawPreview?.openLabel) || (actionType === "navigate_to_screen" ? "Oeffnen" : null),
  };
}
...
function buildFallbackCoachMessage(userInput, risk, contextSummary) {
  ...
  if (risk.pushDespiteRisk || risk.hasFatigue) {
    ...
    const intro = intros[stableVariantIndex(seed, intros.length)];
    const nextKey = contextSummary?.nextKeySession ? `Schiebe die Qualitaet auf ${contextSummary.nextKeySession}.` : "Schiebe die Qualitaet auf den naechsten frischen Tag.";
...
app.get("/api/ai/schema-check", async (_req, res) => {
  const schemaViolations = collectSchemaAdditionalPropertiesViolations(AI_RESPONSE_SCHEMA);
  const requiredCoverageViolations = collectSchemaRequiredCoverageViolations(AI_RESPONSE_SCHEMA);
  return res.json({
    ok: true,
...
app.post("/api/ai/daily-coach", async (req, res) => {
  const { coachContext } = req.body || {};
  if (!coachContext || typeof coachContext !== "object") {
    return res.status(400).json({ error: "Invalid coachContext" });
  }
  if (!client) {
...
app.listen(port, () => {
  ...
  // eslint-disable-next-line no-console
  console.log(
    `[ai-server] schema required coverage ok=${requiredCoverageViolations.length === 0} violations=${requiredCoverageViolations.length}`
  );
});
