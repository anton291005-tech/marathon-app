...
export type PlanDayStreakStatus = "completed" | "incomplete_planned" | "no_training_planned";

/** Plan-aware streak: rest days do not break the chain; incomplete today is skipped. */
export function calculatePlanAwareStreak(
  now: Date,
  statusForDay: (dayStart: Date) => PlanDayStreakStatus,
): number {
  const todayStart = normalizeCalendarDay(now);
  let d = new Date(todayStart);
  let streak = 0;
  ...
  for (let i = 0; i < max; i += 1) {
    ...
    if (status === "no_training_planned") {
      d.setDate(d.getDate() - 1);
      continue;
    }
    if (isCalToday && status === "incomplete_planned") {
      d.setDate(d.getDate() - 1);
      continue;
    }
    break;
