/**
 * AUXILIARY — experimental rule engine (NOT SSOT).
 *
 * Forbidden consumers: AI context, remote coach, primary product UI tied to recovery intelligence.
 * This module does not import RecoveryDomainState. Callers may pass a recovery *label string*
 * that was copied from domain elsewhere — this engine does not validate SSOT.
 *
...
/** Pre-aggregated heuristics only — not a domain snapshot. */
export type DailyDecisionInput = {
  /** Typically aligned with unified recovery label when wired from product code */
  recoveryLabel: string;
  /** From weekly load heuristics — "hoch" | "mittel" | "niedrig" */
...
export function getDailyDecision(input: DailyDecisionInput): DailyDecision {
  ...
  if (isFatigue(recoveryLabel)) {
    loadScore += 3;
    reasons.push("Erholung niedrig");
  } else if (!isFresh(recoveryLabel)) {
    loadScore += 1;
    reasons.push("Erholung normal");
  } else {
    loadScore -= 0.5;
    reasons.push("Erholung stabil");
  }
